...
class _CategoryPieChartState extends State<CategoryPieChart> {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: GestureDetector(
        ...
        onTapDown: (details) {
          final index = _hitTestSlice(details.localPosition, Size(widget.size, widget.size), strokeWidth);
          if (index == null) {
            _hideTooltip();
            return;
          }
  ...
  Widget _buildTooltip(BuildContext context, int index, Offset tapPos, double total, double strokeWidth) {
    ...
    final centerX = tapPos.dx - (tooltipWidth / 2);
    final left = centerX.clamp(0.0, widget.size - tooltipWidth);

    final placeAbove = tapPos.dy > (widget.size / 2);
    final rawTop = placeAbove ? tapPos.dy - tooltipHeight - 8 : tapPos.dy + 8;
    final top = rawTop.clamp(0.0, widget.size - tooltipHeight);
